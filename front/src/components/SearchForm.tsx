          ...
          </div>
          {
            sourceSuggestions.length > 1 && (
              <ul className='absolute top-full w-full'>
                {
                  sourceSuggestions.map((system: any) => (
                    <li className='border-b px-4 py-2' key={system.name} onClick={() => clickSourceSuggestion(system.name)}>
                      {system.name}
                    </li>
                  ))
                }
        ...
        <div className='form-control px-2 w-1/2 lg:w-1/3'>
          ...
          </label>
          {
            destinationSuggestions.length > 1 && (
              <ul className='absolute top-full w-full'>
                {
                  destinationSuggestions.map((system: any) => (
                    <li className='border-b px-4 py-2' key={system.name} onClick={() => clickDestinationSuggestion(system.name)}>
                      {system.name}
                    </li>
                  ))
                }
